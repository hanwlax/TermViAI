...
| 点击顶栏 `+` | 打开 New Tab 页面，展示 Saved workspaces、Recent tab groups 和 Recent connections，不创建本地终端。 |
...
- **Recent tab groups**：记录已组成的多 SSH 窗格布局，便于再次打开；单主机不作为历史组重复展示。
- **Recent connections**：记录已打开的 SSH 主机，按连接身份去重并按时间排列。

点击保存项或历史组时，TermViAI 根据模板重新建立 SSH 连接并按布局分屏。认证仍由原有 SSH 流程完成，可能需要输入密码或确认主机。它不会恢复已退出的远端程序、终端滚屏或曾输入的命令。恢复过程中某个连接失败时显示错误，并保留已经打开的会话。
...
最近连接最多保留 30 条，历史组最多 20 条，手动保存组最多 100 条。保存使用同目录临时文件替换；读取损坏文件或不支持的版本会报错并保留原文件。当前实现不提供远端进程恢复或自动重连。
