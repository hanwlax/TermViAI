...
## 2026-09-18 重连失败与历史文字重叠

重连错误及随后成功连接的首行覆盖历史的直接原因是重新发送了带绝对光标定位的应用更新横幅。重启 reader 现不再重放该横幅；旧 reader 的缓冲输出全部解析完成后才标记可重连。重连同时重置残留的滚动区域、光标原点和输入模式，在可见历史末尾追加，满屏时正常滚动。保留实际 SSH 错误，省略底层进程退出/Hold 重复提示。当前验证及使用行为见 [SSH_RECONNECT.md](SSH_RECONNECT.md)。

## 2026-09-18 SSH 重连修正与整组重连

用户实际遇到 libssh 通用 `Socket error: No error`，旧逻辑只识别 `DeadSession`，导致原位重连和旧组仍打开时的新连接均可能复用失效会话。显式重连现始终建立新 SSH 传输；新连接的缓存 PTY 请求失败后也会重建一次。重连错误改为 pane 专属，避免蔓延到 Hosts/New Tab。刷新图标重绘为 16px 单箭头圆弧；所有 pane 断线时 Tab 提供整组重连按钮。

按用户澄清移除蓝色分隔线，原图蓝线仅为位置标记。保留主屏滚动历史，新会话直接换行衔接。验证与操作边界见 [SSH_RECONNECT.md](SSH_RECONNECT.md)。
