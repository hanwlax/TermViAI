...
impl crate::TermWindow {
    pub fn paint_split(
        ...
        // While the chrome is interpolating to the new grid, its separator
        // location differs from the mux drag target. Avoid capturing content
        // clicks as a split drag until both coordinates agree again.
        if self.termviai_pane_geometry_in_motion() {
            return Ok(());
        }
        let draw_lines = !self.config.termviai_ui || self.termviai_pane_headers().is_empty();
